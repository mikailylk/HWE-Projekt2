...
uint8_t _loc_WhichDisplay = DISPID_NO_DISP;
...
/****************************************************************************************/
/* Der folgende Abschnitt wird nur für das OLED Display benötigt */
/****************************************************************************************/

#ifdef DISP_OLED

/* Variablen und defines für den Zeichensatz									        */
...
// uint8_t fontData7x8(uint8_t ix, uint8_t kx);

// Hier werden alle Zeichen definiert und im Program-memory abgegelegt
const unsigned char fontData7x8pgm[FONT_CHARS*FONT_WIDTH] PROGMEM = {
	0,    0,   0,   0,   0,   0,   0, // ' '  32
	0,    6,  95,  95,   6,   0,   0, // '!'  33
	0,    7,   7,   0,   7,   7,   0, // '"'  34
...
// Auslesen einer Zeile eines Zeichens wobei die Bist umgedreht werden
unsigned char fontDataRotated(unsigned char ixChar, unsigned char ixLine)
{
	
	unsigned char LocLine, kx, BitVal;
	
	LocLine=0;
	
	/* Catch Errors */
	/*ixLine=(ixLine>7) ? ixLine : 7;*/
	
	/* Loop over all Bits, leave Bit0 0 */
	for (kx=1;kx<8;kx++)
	{
		//BitVal=(fontData7x8(ixChar,(7-kx))&(1<<ixLine));
		BitVal=0;
		if (BitVal)
		{
			LocLine=LocLine+(1<<kx);
		}
	}
	return LocLine;
}

// Auslesen einer Zeile eines Zeichens 
uint8_t fontData7x8(uint8_t mychar, uint8_t cnt)
{
	return pgm_read_byte(fontData7x8pgm+mychar*7+cnt);
}
/****************************************************************************************/
/****************************************************************************************/
...
/****************************************************************************************/
/* I2C für selbstdefinierte Pins													    */
/* Kopie des Codes von Leo																*/
/****************************************************************************************/
...
// TWI schreiben
uint8_t twi_disp_write(uint8_t data);

// Start TWI
uint8_t twi_disp_start(uint8_t address)
{
/* SCL ''''\...
   SDA '\......
*/
   ...
   return twi_disp_write(address);

}

// Stop TWI
void twi_disp_stop(void)
{
/* SCL ..../''''''
   SDA '\....../''
*/   
...
uint8_t twi_disp_read(uint8_t ack)
{
   uint8_t data = 0;
   uint8_t i;
...
#if  defined(DISP_MEGACARD)||defined(DISP_MEGACARD_MOD)
...
void _hw_zToLCD(char dataD, uint8_t IsData)
{
	...
	DISP_PORTENRS&=~ (1<<DISP_RS | 1<<DISP_EN);
	
	// Datenleitungen auf Ausgang setzen und löschen
	//DDRB|=(1<<DISP_DB4 | 1<<DISP_DB5 | 1<<DISP_DB6 | 1<<DISP_DB7);
	DISP_PORTDATA&=~(1<<DISP_DB4 | 1<<DISP_DB5 | 1<<DISP_DB6 | 1<<DISP_DB7);

	// und wieder gesetzt entsprechend den Daten
	if (dataD & 0x10) DISP_PORTDATA|=1<<DISP_DB4;
	if (dataD & 0x20) DISP_PORTDATA|=1<<DISP_DB5;
	if (dataD & 0x40) DISP_PORTDATA|=1<<DISP_DB6;
	if (dataD & 0x80) DISP_PORTDATA|=1<<DISP_DB7;
	
	// Setze das Daten/Command flag
	if (IsData)
	{
		DISP_PORTENRS|=1<<DISP_RS;
	}

	// EN High->Low ausführen des Befehls
	DISP_PORTENRS|=1<< DISP_EN;
	_delay_us(1);
	...
	// und wieder gesetzt entsprechend den Daten
	if (dataD & 0x01) DISP_PORTDATA|=1<<DISP_DB4;
	if (dataD & 0x02) DISP_PORTDATA|=1<<DISP_DB5;
	if (dataD & 0x04) DISP_PORTDATA|=1<<DISP_DB6;
	if (dataD & 0x08) DISP_PORTDATA|=1<<DISP_DB7;
	if (IsData)
	{
		DISP_PORTENRS|=1<<DISP_RS;
	}
...
void _hw_Init(void)
{
...
#ifdef DISP_MC_NEU
	// Für das neue Display mit Port Expander
	...
	// Testen, ob das neue Modul angeschlossen ist oder das alte.
	// Es wird geprüft, ob PD6 und PD7 hochohmig verbunden sind (100k)
	//DD_old=DDRD;
	//PORT_old=PORTD;
	//DDRD|=(1<<6);
	//DDRD&=~(1<<7);
	...
	DISP_DDRENRS|= 1<<DISP_RS | 1<<DISP_EN ;
	DISP_DDRDATA|= 1<<DISP_DB7 | 1<<DISP_DB6 | 1<<DISP_DB5 | 1<<DISP_DB4;
	
	
	// Umschalten auf 4-Bit Display
	// Datenleitungen löschen
	DISP_PORTDATA&=~(1<<DISP_DB7 | 1<<DISP_DB6 | 1<<DISP_DB5 | 1<<DISP_DB4);
	// RS=0 und EN =0 
	DISP_PORTENRS&=~(1<<DISP_RS | 1<<DISP_EN);
	
	// Datenleitungen vorbereiten (Datenblatt Controller)
	DISP_PORTDATA|=(1<<DISP_DB4) | (1<<DISP_DB5);
...
#else
	#define OLED_DEV_ADDR 0b00111100
#endif
...
void _hw_Init(void)
{
	...
	// Display einschalten
	// Bus öffnen
	twi_disp_start(OLED_DEV_ADDR);
	
	// Daten schreiben
	twi_disp_write(OLED_DEV_ADDR>>1);
	...
	// Daten schreiben
	twi_disp_write(OLED_CTRL_BYTE_CMD);
	...
	// Turn Display on
	i2c_WriteByteToRegister (OLED_DEV_ADDR, OLED_DEV_ADDR, 0xaf);
	_delay_ms(100);

	// Enable charge pump regulator
	i2c_WriteTwoBytesToRegister (OLED_DEV_ADDR, OLED_CTRL_BYTE_CMD, 0x8d, 0x14);
	_delay_ms(100);

	// Enable RAM Content
	i2c_WriteByteToRegister (OLED_DEV_ADDR, OLED_CTRL_BYTE_CMD, 0xa4);
	_delay_ms(100);
...
void oled_pos_full(unsigned char col, unsigned char row)
{
	...
	// Daten schreiben
	twi_disp_write((0x10 | ((col>>1) & 0x0f)));
	...
	#else
	// Set Page Adress (=row)
	i2c_WriteByteToRegister (OLED_DEV_ADDR, OLED_CTRL_BYTE_CMD, (0b10110000 | row));
	
	// Lower nibble of col adresse. The start adress is the col multiplied by 8
	i2c_WriteByteToRegister (OLED_DEV_ADDR, OLED_CTRL_BYTE_CMD, (col<<3) & 0x0f);

	// Higher nibble of col adresse. The start adress is the col multiplied by 8
	i2c_WriteByteToRegister (OLED_DEV_ADDR, OLED_CTRL_BYTE_CMD, (0x10 | ((col>>1) & 0x0f)));
	#endif
}

void _hw_Pos(unsigned char col, unsigned char row)
{
	if (row>=OLED_USER_LINES) row=OLED_USER_LINES;
	oled_pos_full(col,row+OLED_FIRST_USER_LINE);
}
...
void oled_clear_full()
{
	...
	for(ix=0;ix<OLED_ROWS;ix++)
	{
		...
		for(jx=0;jx<OLED_PIXEL_X;jx++)
		{
				...
				// Daten schreiben
				twi_disp_write(OLED_CTRL_BYTE_DATA);
			...
			#else
			// write zero to next GDRAM Position
				i2c_WriteByteToRegister (OLED_DEV_ADDR, OLED_CTRL_BYTE_DATA, 0);
			#endif
		}
	}
...
void _hw_Clear()
{
	...
	// loop over all user line pages
	for(ix=0;ix<OLED_USER_LINES;ix++)
	{
		//Set cursor to beginning of next page
		_hw_Pos(0,ix);
	...
	// finally we set the cursor back to 0,0
	display_Pos(0,0);
}
...
void _hw_CharToDisplay (uint8_t c)
{
	...
	for (cnt=0;cnt<7;cnt++)
	{
			...
			// Daten schreiben
			twi_disp_write(fontData7x8(c,cnt));
		...
		#else
			i2c_WriteByteToRegister (OLED_DEV_ADDR, OLED_CTRL_BYTE_DATA, fontData7x8(c,cnt));
		#endif
	}
